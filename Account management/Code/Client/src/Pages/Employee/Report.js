...
import VehicleReportpopUpTable from "../../component/Table/VehicalReportTable";
...
async function fetchClaims() {
  const res = await publicRequest.get(`VehicalReport/all`);
  return res;
}

const VehicalReportTable = () => {
  ...
  const [columns, setColumns] = useState([
    ...
    },
    {
      ...
      sorting: false,
      render: (rowdata) => {
        // console.log(rowdata.TotalProfit);
        return numberWithCommas(rowdata.TotalProfit);
      },
    },
  ]);
  ...
  const deleteClaims = (id) => {
    console.log(id);
    return publicRequest.delete(`VehicalReport/delete/${id}`);
  };
  ...
  const deleteReq = useMutation(deleteClaims, {
    onSuccess: () => {
      ...
      queryClient.invalidateQueries("VehicalReport");
    },
    onError: (error) => {
      setNotify({
        isOpen: true,
  ...
  return (
    <div className={classes.roots} id="review">
      <Container component="main" maxWidth="xl" className={classes.main}>
        ...
        >
          {console.log(params)}
          <VehicleReportpopUpTable props={params} />
        </PopUp>
...
export default VehicalReportTable;
